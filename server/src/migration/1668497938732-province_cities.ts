...
export class provinceCities1668497938732 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        for (let i = 0; i < this.metaData.length; i++) {
            ...
            const ifCityExist = await locationRepo.count({ city: this.metaData[i].city })
            if (!ifCityExist) {
                location.city = this.metaData[i].city
                location.city_ascii = this.metaData[i].city_ascii
                location.province_id = this.metaData[i].province_id
                location.province_name = this.metaData[i].province_name
                location.lat = this.metaData[i].lat.toString()
                location.lng = this.metaData[i].lng.toString()
                location.population = this.metaData[i].population.toString()
                location.density = this.metaData[i].density.toString()
                location.timezone = this.metaData[i].timezone
                location.ranking = this.metaData[i].ranking
                location.postal = this.metaData[i].postal
                location.city_id = this.metaData[i].id.toString()
                queryRunner.manager.save(location)
            }
            // if (!ifService) {
