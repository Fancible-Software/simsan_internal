...
export class DashboardController {
    ...
    async users(
        ...
        try {
            ...
            const adminCount = await adminRepo.count({ where: { is_verified: true } })


            const { sum } = await feedbackRepo.createQueryBuilder('form').select("SUM(CAST(form.final_amount AS DECIMAL))", "sum").getRawOne()
